...
public class AcompanhanteParser {
	...
	public List<Acompanhante> lerTodosAcompanhantes(String todosAcompanhantesJson) throws ParseException {

		List<Acompanhante> listacompanhantes = new ArrayList<Acompanhante>();
		try {

			JSONObject json = new JSONObject(todosAcompanhantesJson);
			if (json.has("objacompanhante") && json.optJSONArray("objacompanhante") != null) {
				JSONArray acompanhanteArray = json.getJSONArray("objacompanhante");
				listacompanhantes = parserAcompanhanteArray(acompanhanteArray);
			} else {
				listacompanhantes = new ArrayList<Acompanhante>();
				listacompanhantes.add(parseAcompanhanteJsonObject(json.getJSONObject("objacompanhante")));
			}
	...
	private List<Acompanhante> parserAcompanhanteArray(JSONArray acompanhanteArray) throws ParseException {
		List<Acompanhante> listacompanhante = new ArrayList<Acompanhante>();

		for (int i = 0; i < acompanhanteArray.length(); i++) {
			try {

				listacompanhante.add(parseAcompanhanteJsonObject(acompanhanteArray.getJSONObject(i)));
	...
	public static Acompanhante parseAcompanhanteJsonObject(JSONObject usuarioJson) throws JSONException {
		...
		acomp.setHorario_atentimento(usuarioJson.optString("horario_atendimento"));
